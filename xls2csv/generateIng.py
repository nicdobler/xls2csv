...
for inputExcelFile in glob.iglob(path + "/Movements.xls"):
    ...
    excelFile = pd.read_excel(path + "Movements.xls", header=4, engine="xlrd")
    ...
    csvFile['memo'] = excelFile[['CATEGORÍA', 'SUBCATEGORÍA']].\
        agg('/'.join, axis=1) \
        if excelFile["DESCRIPCIÓN"].str.startswith('Pago en') \
        else ""
